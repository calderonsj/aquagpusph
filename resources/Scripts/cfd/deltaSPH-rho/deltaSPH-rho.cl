                   ...
                   usize N)
{
    ...
    FOR_NEIGHS(N, jhoc){
	    ...
	    const float r2 = (q * q + 0.01f) * H * H;
            ...
            _LAPP_ += 2.f * (rho[j] - rho_i) * dot(r_ij, f_ij * r_ij) / r2;
        }
    }END_FOR_NEIGHS()
                        ...
                        usize N)
{
    ...
    FOR_NEIGHS(N, jhoc){
	    ...
	    const float r2 = (q * q + 0.01f) * H * H;
            ...
            _LAPP_ -= dot(gradp_ij, r_ij) * dot(r_ij, f_ij * r_ij) / r2;
        }
    }END_FOR_NEIGHS()
		    ...
		    const float cs)
{
    ...
    const float delta_f = delta[set_i] * h * cs;
